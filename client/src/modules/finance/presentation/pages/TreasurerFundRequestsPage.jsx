...
const TreasurerFundRequestsPage = () => {
  ...
  const release = async (id, suggestedAmount) => {
    const amountReleased = window.prompt(
      "Amount to release",
      String(suggestedAmount || 0),
    );
    if (!amountReleased) return;
    await treasurerUseCases.releaseFundRequest(id, {
              ...
              <tr key={fr._id} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 space-x-2">
                  <button
                    ...
                    onClick={() => approve(fr._id)}
                  >
                    Approve
                  </button>
                  <button
                    className="rounded bg-red-600 px-2 py-1 text-xs text-white"
                    onClick={() => reject(fr._id)}
                  >
                    Reject
                  </button>
                  <button
                    className="rounded border border-slate-300 px-2 py-1 text-xs"
                    onClick={() => release(fr._id, fr.amountRequested)}
                  >
                    Release
                  </button>
                </td>
