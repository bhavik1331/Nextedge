  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={readAll} className="rounded border border-slate-300 px-3 py-2 text-sm">Mark all read</button>
      </div>
        ...
        {notifications.map((n) => (
          <article key={n._id} className={`rounded-lg border p-3 ${n.isRead ? "border-slate-200 bg-slate-50" : "border-blue-200 bg-blue-50"}`}>
            <div className="flex items-start justify-between gap-3">
              ...
              {!n.isRead && <button className="rounded bg-slate-900 px-2 py-1 text-xs text-white" onClick={() => read(n._id)}>Read</button>}
            </div>
          </article>
        ))}
      </div>
