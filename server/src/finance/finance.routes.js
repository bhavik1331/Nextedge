...
const treasurerGuard = [authenticateMember, authorizeRole("TREASURER", "ADMIN")];
const clubHeadGuard = [authenticateMember, authorizeRole("CLUB_HEAD", "ADMIN")];
const memberGuard = [authenticateMember, authorizeRole("MEMBER", "TREASURER", "CLUB_HEAD", "ADMIN")];
...
router.post(
  "/payments",
  ...memberGuard,
  upload.fields([{ name: "proofFile", maxCount: 1 }, { name: "proofImage", maxCount: 1 }]),
  submitPayment
);
...
router.patch("/payments/:id/clarify", ...treasurerGuard, requestPaymentClarification);
...
router.patch("/fund-requests/:id/approve", ...treasurerGuard, approveFundRequest);
router.patch("/fund-requests/:id/reject", ...treasurerGuard, rejectFundRequest);
router.patch("/fund-requests/:id/release", ...treasurerGuard, releaseFundRequest);

router.post("/expenses", ...treasurerGuard, upload.fields([{ name: "receiptFile", maxCount: 1 }]), createExpense);
...
router.patch("/notifications/:id/read", authenticateMember, markNotificationRead);
router.patch("/notifications/read-all", authenticateMember, markAllNotificationsRead);
