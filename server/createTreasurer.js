...
const usage = () => {
  console.log("Usage:");
  console.log(
    '  node createTreasurer.js --name "Treasurer Name" --email treasurer@example.com --password "StrongPass123"',
  );
  console.log("Optional:");
  console.log(
    "  --force   Update existing member with same email to TREASURER and reset name/password",
  );
};
...
const main = async () => {
  ...
  try {
    ...
    if (existing && !force) {
      console.error(
        "A member with this email already exists. Use --force to update it as TREASURER.",
      );
      process.exit(1);
    }
